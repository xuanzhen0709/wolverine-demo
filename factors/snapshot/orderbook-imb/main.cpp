...
//
// 目标：与 stats/mid 对齐的低频横截面因子。每天在 8 个固定 30 分钟时点
//   （09:30 / 10:00 / 10:30 / 11:00 / 13:00 / 13:30 / 14:00 / 14:30）各输出一次。
//
// 计算逻辑：
...
//   3. 到达 8 个报出时点 t 时，对窗口内的切片 OBI 序列算 4 个汇总量并各自落盘：
//         mean  —— 等权均值（每个切片权重相同）
...
//   每个 csv：表头 exchtime,localtime,<symbol...>；8 个数据行；缺失写 "nan"。
//   4 个文件的 exchtime/localtime 两列与 stats/mid 逐行一致（同样的报出时点门控），
//   因此可直接喂给 compute_label.py / cs_ic_calculator 做跨日 label 与 IC。
//
// 无 look-ahead：日内窗口 / 时长一律用 exchtime（当日纳秒）；localtime（epoch）仅写列。
//
// 可选配置（signal.config）：
...
//   label_times      : 覆盖默认 8 个报出时点（HHMMSS 整数数组）
...
// 默认 8 个截面时间点（HHMMSS，交易所时间），与 stats/mid 对齐。
...
void Signal::initialize(const Config *root)
{
  ...
  wllog_info("orderbook-imb: prefix={}, window={}s, depth_levels={}({}), "
             "{} label times, out={}/{}/<stat>/<date>/\n",
             prefix_, window_sec, depth_levels_,
             depth_levels_ == 0 ? "all" : "topK", targets_.size(), output_dir_,
             prefix_);
}
...
void Signal::on_cs_snapshot(const CsSnapshotEvent *ev)
{
  ...
  const int64_t cutoff = ex - window_ns_; // 早于此的切片移出窗口
  for (std::uint16_t i = 0; i < ev->ins_nr; ++i) {
    auto &dq = windows_[i];
    // 先裁旧（无论是否纳入本切片，都保持「窗口 = 过去 window_sec」）。
    while (!dq.empty() && dq.front().exchtime < cutoff) dq.pop_front();
...
Agg Signal::aggregate(const std::deque<Slice> &dq, int64_t t_emit)
{
  ...
  for (std::size_t k = 0; k < n; ++k) {
    const int64_t nxt = (k + 1 < n) ? dq[k + 1].exchtime : t_emit;
    double w = static_cast<double>(nxt - dq[k].exchtime);
