...
from run_pnl_stats import run_pnl_stats
...
class SingalCfg:
    ...
    def run(
        ...
        self.outdir = (
            outdir_root.resolve()
            / f"""PnL.{self.name}.{self.start}.{self.end}.{"-".join(exec_price)}"""
        )
        self.outdir.mkdir(parents=True, exist_ok=True)

        outcfg_file: Path = (
            self.outdir.resolve()
            / f"""PnL.{self.name}.{self.start}.{self.end}.{"-".join(exec_price)}.yml"""
        )
    ...
    def run_stats(self, infile: Path, start: int, end: int, rel_sig_dir: Path):
        output = self.outdir.joinpath(f"{self.name}_stats")
...
def main():
    parser = argparse.ArgumentParser("pnl calculator")
    parser.add_argument(
        "signal_config",
        type=Path,
    ...
    cfg = SingalCfg(args.signal_config, args.start, args.end)
    wlsim_ret = cfg.run(
        args.output, args.exec_price, "daily", args.stock_map, args.rel_sig_dir
    )
