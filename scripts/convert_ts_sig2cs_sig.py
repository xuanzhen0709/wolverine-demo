...
class SingalCfg:
    ...
    def make_cs_sigs(self, map_file: Path, output_root: Path,
                     ignore_missing: bool):
        if len(self.sigcfg["targets"]) != 1:
            raise RuntimeError("multi target!")
        target = self.sigcfg["targets"][0]
        map_info: pd.DataFrame = pd.read_excel(
            map_file, header=None, sheet_name=target)
        ...
        for date in date_list:
            ...
            new_sig_df.to_csv(output_dir.joinpath(
                f"cs_sig.{self.name}-{date}.csv"), index=False)
