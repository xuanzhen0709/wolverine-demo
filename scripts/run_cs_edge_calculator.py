...
class SignalCfg:
    ...
    def run(self, outdir_root: Path, quantile: float, future_bias_start: str, future_bias_end: str, future_bias_step: str, rel_sig_dir:Path)-> subprocess.CompletedProcess:
        ...
        self.outdir: Path = outdir_root.resolve(
        ) / f"cs_edge.{self.out_name}.{self.start}.{self.end}.{quantile*100}%"
        self.outdir.mkdir(parents=True, exist_ok=True)

        outcfg_file: Path = self.outdir.resolve(
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        help=
        "proportion of extreme values, the range is: (0,1], default: 0.1"
    )
    parser.add_argument(
        "--future-bias-start",
        type=str,
        required=True,
        help=
        "future bias start, postfixes such as 's' 'm' and 'h' are supported"
    )
    parser.add_argument(
        "--future-bias-end",
        type=str,
        required=True,
        help=
        "future bias end, postfixes such as 's' 'm' and 'h' are supported"
    )
    parser.add_argument(
        "--future-bias-step",
        type=str,
        required=True,
        help=
        "future bias step, postfixes such as 's' 'm' and 'h' are supported"
    )
    parser.add_argument(
    ...
    cfg = SignalCfg(args.signal_config, args.name, args.start, args.end,
                    args.use_system_uv)
    wlsim_ret = cfg.run(args.output, args.quantile, args.future_bias_start,args.future_bias_end, args.future_bias_step, args.rel_sig_dir)
